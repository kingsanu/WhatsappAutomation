...
export class SessionController {
  ...
  async getQRCode(@Param('username') username: string, @Res() res: Response) {
    try {
      ...
      if (sessionInfo) {
        this.logger.log(`Existing session found for ${username}, preserving for persistence`);
      }
  ...
  async setSessionPersistence(
    ...
    try {
      const { policy = PersistencePolicy.PERMANENT, autoReconnect = true } = body;

      await this.whatsappService.setSessionPersistence(username, policy, autoReconnect);
  ...
  async getDetailedSessionInfo(@Param('username') username: string) {
    try {
      const sessionInfo = await this.whatsappService.getDetailedSessionInfo(username);
      return {
        success: true,
        data: sessionInfo
      };
  ...
  async restoreFromBackup(
    ...
    try {
      const result = await this.sessionBackupService.restoreFromBackup(backupId, body);
      return {
        success: true,
  ...
  async resolveSessionConflict(@Param('username') username: string) {
    try {
      const result = await this.whatsappService.resolveSessionConflict(username);
      return {
        success: true,
  ...
  async forceSessionTakeover(
    ...
    try {
      ...
      const result = await this.whatsappService.forceSessionTakeover(username, completeReset);
      return {
        success: true,
  ...
  async activateExistingSession(@Param('username') username: string) {
    try {
      ...
      return {
        ...
        data: {
          username,
          message: 'Session activated using stored credentials - no QR scan required',
          authPreserved: true
        }
      };
    } catch (error) {
  ...
  async getSerializationStats() {
    try {
      const stats = await this.persistentAuthStateService.getSerializationStats();
      return {
        success: true,
        data: stats
      };
  ...
  async getSerializationMetrics(@Param('username') username: string) {
    try {
      const metrics = this.persistentAuthStateService.getSerializationMetrics(username);
      return {
        success: true,
  ...
  async validateSerializationIntegrity(@Param('username') username: string) {
    try {
      const validation = await this.persistentAuthStateService.validateSerializationIntegrity(username);
      return {
        success: true,
        data: validation
      };
